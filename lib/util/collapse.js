...
exports = module.exports = function (parent, pk, options, data) {
    ...
    var mapping = data.reduce(function (acc, row) {
        return (function build (obj, schema, parents, name) {
            ...
            var pkField = name + separator + (opts.pk || pk);

            if (parents.length) {
                pkField = parents.join(separator) + separator + pkField;	// anything deeper than child__id needs to build the full column name
            }
        ...
        return Object.keys(map).reduce(function (acc, k) {
            Object.keys(schema)
                ...
                .forEach(function (c) {
                    ...
                    var accumulator = options[c] && options[c].isArray ? [] : {};
